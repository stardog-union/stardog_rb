...
class DbTest < Minitest::Test
  ...
  def test_db_list
    response = StardogRb::Db.list(@conn)
  ...
  def test_db_create
    response = StardogRb::Db.create(@conn, 'test_db')
    response_json = JSON.parse(response.body)
  ...
  def test_db_create_with_options
    ...
    response = StardogRb::Db.create(@conn, 'test_db', options)
    response_json = JSON.parse(response.body)
    message = response_json['message']
    assert response.code == '201'
    assert response.content_type == 'application/json'
  ...
  def test_db_drop
    response = StardogRb::Db.drop(@conn, 'test_db')
    response_json = JSON.parse(response.body)
    message = response_json['message']
    assert response.code == '200'
    assert response.content_type == 'application/json'
  ...
  def test_db_online
    response = StardogRb::Db.online(@conn, 'test_db')
    response_json = JSON.parse(response.body)
    message = response_json['message']
    assert response.code == '200'
    assert response.content_type == 'application/json'
  ...
  def test_db_offline
    response = StardogRb::Db.offline(@conn, 'test_db')
    response_json = JSON.parse(response.body)
    message = response_json['message']
    assert response.code == '200'
    assert response.content_type == 'application/json'
  ...
  def test_db_set_options
    ...
    response = StardogRb::Db.set_options(@conn, 'test_db', options)
    assert response.code == '200'
  ...
  def test_db_get_options
    ...
    response = StardogRb::Db.get_options(@conn, 'test_db', options)
    response_json = JSON.parse(response.body)
    assert response.code == '200'
